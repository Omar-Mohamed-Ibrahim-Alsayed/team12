...
int allocflags; /* allocflags for alloc_mem pre-defined funciton that manage actual 
...
uint64_t handleLevel(uint64_t **curPagingTable, uint64_t curLevelBits, unsigned int readOnly) {
...
if (readOnly == 1) return *finalRoot == 0 ? NO_MAPPING : *finalRoot;
 ...
 if (handleLevel(&curPagingTable, curLevelBits, readOnly) == NO_MAPPING) return 
NO_MAPPING;
 }
 }
 return NO_MAPPING;
